# Tutor Agent Application

  The Dialog starter application in Java is a simple application that demonstrates how the IBM Watson works as a tutoring agent to help understand basic concepts. 

**This application requires a tutor agent dialog file to be preloaded. You can use the** [dialog-tool](https://github.com/watson-developer-cloud/dialog-tool) **to create a dialog.**
...
## Getting started
  ...
  3. Edit the `manifest.yml` file and replace `<application-name>` with a unique name for your copy of the application. The name that you specify determines the application's URL, such as `<application-name>.mybluemix.net`.
    ...
    - services:
      ...
      path: dialog-java-local-intelli-tutor.war
      memory: 512M
    ```
